...
7. [第7章 関数](#第7章-関数)
    ...
    - [7.4 再帰的定義$\dag$](#74-再帰的定義)
    - [7.5 ローカル変数とグローバル変数$\dag$](#75-ローカル変数とグローバル変数)
    - [第7章の演習問題](#第7章の演習問題)
...
3. [^1]:パソコンに文字で指示を出す画面（プログラムを動かすための入力画面）
...
文字列どうしを結合する場合は + 演算子で文字列をつなぎます．
#### example_1_3_2.py
``` Python
print("Hello " + "World!") # Hello World! と表示されます
```

文字列と数値を + 演算子で結合することはできませんが，次のようにしてつなげて表示することができます．
#### example_1_3_3.py
``` python
print(1, "o`clock") # 1 o`clock と表示されます
```
...
## 3.5 辞書 $\dag$
### 辞書
辞書は、キーと値のペアでデータを管理します。キーを使って値にアクセスします。
#### 辞書の定義
``` python
...
### 整数 (int)
整数は，小数点のない数値です．整数には加算（+），減算（-），乗算（*），除算（/），剰余（%），累乗（**）などの演算が可能です．
...
for i in range(5):
    print(i)
```
この例では，`range(5)` により 0 から 5 の直前までの整数が順に i に代入され，表示されます．
...
for i in range(3,10):
    print(i)
```
この例では，`range(3,10)` により 3 から 10 の直前までの整数が順に i に代入され，表示されます．
...
for i in range(10,0,-2):
    print(i)
```
この例では，`range(10,0,-2)` により 10 から 0 の直前まで -2 ずつ増やしながら順に i に代入され，表示されます．
