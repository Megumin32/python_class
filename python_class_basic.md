...
my_function()
# print(local_var)  # エラー: local_varはローカル変数なので，関数外ではアクセスできない
```
...
my_function()
print(global_var) # global_varはグローバル変数なので，どこからでもアクセスできる
```
...
##### practice6_02.py
引数としてリストを受け取り，そのリスト内のすべての要素の合計を計算して返す関数`sum_list()`を定義してください．（リストの要素の合計を求めるメソッドを調べて下さい．）
``` python
def sum_list(array):
    # ここにコードを追加
...
Pythonのモジュールを使うと，1つのファイルに定義されたコードを他のファイルから簡単に再利用することができます．
...
``` python
import math   # mathモジュールを使えるようにする
...
`math`モジュールには，数学に関連する多くの関数（メソッド）が含まれています．平方根や階乗，三角関数などを簡単に計算できます．
`math`モジュールでよく使用される関数には，`sqrt()`, `factorial()` などがあります．
...
##### question7_03.py
"グー", "チョキ", "パー"のいずれかをランダムで表示するプログラムを作成しなさい
``` python
import random
hand = ["グー", "チョキ", "パー"]
# ここにコードを追加
...
##### question7_04.py
1から6までのランダムな数値を5つ生成し，それを表示するプログラムを作成しなさい．
``` python
import random
# ここにコードを追加
```
...
##### practice8_02.py
紙を42回折ることができると，その高さは月に届くという．紙の厚さを0.1mm，月までの距離を38万kmとしてこれを確かめなさい．
また，富士山の高さを超えるためには最低何回折ればよいか求めなさい．
``` python
# 単位をメートルで統一
paper = 0.0001       
moon = 380000000 
